...
namespace Microsoft.ProjectOxford.Face
{
    ...
    public class FaceServiceClient : IFaceServiceClient
    {
        ...
        /// <param name="analyzesHeadPose">If set to <c>true</c> [analyzes head pose].</param>
        /// <returns>The detected faces.</returns>
        public async Task<Face[]> DetectAsync(Stream imageStream, bool analyzesFaceLandmarks = false, bool analyzesAge = false, bool analyzesGender = false, bool analyzesfacialHair = false)
        {
            var requestUrl = string.Format(
                "{0}/{1}?returnFaceAttributes=facialHair{2}&analyzesAge={3}&analyzesGender={4}&facialHair={5}&{6}={7}",
                ServiceHost,
                DetectionsQuery,
                analyzesFaceLandmarks,
                analyzesAge,
        ...
        public async Task<string> DetectAllAsync(Stream imageStream, bool analyzesFaceLandmarks = true, bool analyzesAge = true, bool analyzesGender = true, bool analyzesfacialHair = true)
        {

            var requestUrl = string.Format(" https://api.projectoxford.ai/face/v1.0/detect?returnFaceAttributes=Age,gender,facialHair&{6}={7}",
                ServiceHost,
                DetectionsQuery,
               analyzesFaceLandmarks,
               analyzesAge,
            ...
            return await this.SendRequestReturnStringAsync<Stream, String>(HttpMethod.Post, requestUrl, imageStream);
        }
        ...
        private async Task<TResponse> SendRequestAsync<TRequest, TResponse>(HttpMethod httpMethod, string requestUrl, TRequest requestBody)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                {
                    var xx = JsonConvert.SerializeObject(responseContent);
        ...
        private async Task<string> SendRequestReturnStringAsync<TRequest, String>(HttpMethod httpMethod, string requestUrl, TRequest requestBody)
        {
            var request = new HttpRequestMessage(httpMethod, ServiceHost);
            request.RequestUri = new Uri(requestUrl);
            if (requestBody != null)
            {
                ...
                {                 
                    ...
                    // byte[] m_Bytes = ReadToEnd(requestBody as Stream);
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                if (!string.IsNullOrWhiteSpace(responseContent))
                {
                    var a = responseContent;
                    string xx = JsonConvert.SerializeObject(responseContent).ToString();
                    return xx;                 
                }
